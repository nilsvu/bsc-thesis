...
\plt{varying_mg}{Tensor perturbations with non-zero mass parameter~\(\mofmetr{g}\)}{}


\section{Decoupling at early times}

Bimetric modified gravity theories, like unimetric theories, usually deviate from \LCDM only at late times to provide an accelerated expansion of the universe without a cosmological constant. \todo{add reference} This is discussed in more detail in \autoref{sec:param_friction_late}. For physically viable bimetric theories it is therefore reasonable to assume that the metrics decouple in early-time cosmological regimes where the physical metric~\(g\) behaves unmodified but the reference metric can exhibit arbitrary dynamics.
