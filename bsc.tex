...
\end{equation}
will also be small and we obtain \autocite{Schuller}
...
Because of its symmetry condition\todo{add reference or derive}, \(\delta g\) has 10 degrees of freedom that we can parametrize as \autocite{Schuller}
\begin{align}
...
\begin{equation}
	...
	\end{bmatrix}_{ij}
\end{equation}
with an implicit choice of the \(z\)-axis in direction of the \emph{wave vector}~\(\vect{k}\) \autocite{Dodelson}. \todo{elaborate choice of axis, show div/trace-free with wave-vector k, "transverse-traceless TT gauge"}
...
\plt{grav_wave_deform}{Tensorial deformation of spacetime}{A passing gravitational wave alternatingly stretches and compresses the proper distance between two masses (visualized as points) in the plane transverse to its propagation direction \autocite{Schutz}. The two polarizations \(\hcross\) and \(\hplus\) are rotated by \ang{45} relative to one another.}
...
\begin{equation}
	...
	\eqpunct{.}
\end{equation}
It is a common misconception that the effect of gravitational waves is unobservable since any ruler to measure changes in distance would also be distorted. In an inertial reference frame where two masses are at rest relative to each other, A passing gravitational wave merely induces a force on known as \emph{tidal force}. Since other the electromagnetic interaction that 
...
Already in basic quantum mechanics, the uncertainty principle requires every physical system to have a zero-point energy\addref. This immediately carries over to quantum field theory, where a (free) field is an infinite collection of coupled quantum mechanical harmonic oscillators. With a zero-point energy each, they combine to an infinite \emph{vacuum energy}~\(\rho_\textnormal{vac}\) \autocite{Martin2012}. \todo{add some derivation?}
 
In quantum field theory, the vacuum energy is largely ignored as only differences in energy determine the dynamics of the system. In general relativity, however, all energy content gravitates, including the vacuum energy \autocite{Martin2012,Burgess2013,Padilla2015}. \todo{how about Lamb shift, Casimir effect?}
...
\todo{how about naturalness <-> anthropic  \autocite{Padilla2015}?}
...
\end{align}
\end{subequations}
where both \(\alphaM\) and \(\cT\) may in general be functions of time and depend on the specific model. For instance, \textcite{Hwang1996} show that the additional friction contribution in an \fR{theory} is given by
\begin{equation}
...
I will first consider a constant friction term \(\alphaM=\const\) in the \namedeqref{parametrized evolution equation}{eq:evolution_friction}. \todo{why?}


\subsection{Numerical solution}

A solution of the \namedeqref{parametrized evolution equation}{eq:evolution_friction} for various values of \(\alphaM\) was obtained numerically with the \mmainline{ParametricNDSolve} function in Mathematica under the following assumptions:
...
The \namedeqref{parametrized evolution equation}{eq:evolution_friction} can be solved analytically in cosmological regimes dominated by one of the matter types discussed in \autoref{sec:frw}. The expansion of an \FLRW{} universe dominated by matter or radiation is given by \eqref{eq:frw_a_evol} or
\begin{equation}
